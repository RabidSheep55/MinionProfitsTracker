...
# Setup colourmap with dataset
# This is absolute garbage but it works
rawVals = [[j[1] for j in i] for i in ranks.values()]
flatten = lambda l: [item for sublist in l for item in sublist]
rawVals = flatten(rawVals)
minima = min(rawVals)
maxima = max(rawVals)
...
mapper.set_array([minima, maxima])
...
for m in initOrder:
    ...
    ax.plot(lvls, rank, lw=3, clip_on=False, zorder=0)
    ax.scatter(lvls, rank, clip_on=False, zorder=10, s=200)
    ax.scatter(lvls, rank, clip_on=False, zorder=10, s=60, c=mapper.to_rgba(vals))
...
plt.xticks(lvls)
plt.tick_params(axis="both", bottom=False, left=False, labeltop=True, labelbottom=True) #, labelbottom=False, labelleft=False)
...
# Colorbar
cbar = fig.colorbar(mapper, pad=-0.03, aspect=40, drawedges=False, ax=ax, ticks=np.linspace(minima, maxima, 15))
cbar.ax.set_ylabel("Coins gained per hour per minion")
...
fig.savefig(join('Raw Figs','MinionLvlVsRank-NodeColours.png'), dpi=300, facecolor=bg, edgecolor=bg)
plt.show()
